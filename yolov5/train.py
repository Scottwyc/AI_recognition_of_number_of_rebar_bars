...
def train(hyp, opt, device, callbacks):
    ...
    # Trainloaderpython -c "import torch; print(torch.__version__)"
...
def parse_opt(known=False):
    ...
    # maybe the weight can use previous best result weight?
    parser.add_argument('--weights', type=str, default= ROOT / 'yolov5s.pt', help='initial weights path')

    parser.add_argument('--cfg', type=str, default='own_data/yolov5s.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='own_data/bar.yaml', help='dataset.yaml path')
...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional)
        ...
        #     hyp = yaml.safe_load(f)  # load hyps dict
        with open(opt.hyp, "rb", encoding='utf-8') as f:
            hyp = yaml.safe_load(f)  # load hyps dict
        ...
        if opt.resume_evolve is not None:
            ...
            # with open(ROOT / opt.resume_evolve, "rb", errors="ignore") as f:
            #     evolve_population = yaml.safe_load(f)
            with open(ROOT / opt.resume_evolve, "rb") as f:
                evolve_population = yaml.safe_load(f)
        ...
        # If not resuming from a previous checkpoint, generate initial values from .yaml files in opt.evolve_population
            ...
            for file_name in yaml_files:
                with open(os.path.join(opt.evolve_population, file_name), "rb") as yaml_file:
